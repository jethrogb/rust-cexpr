...
fn file_visit_macros<F: FnMut(Vec<u8>, &[Token])>(file: &str, tokenize_range_adjust: bool, mut visitor: F) {
	unsafe {
		let tu={
		...
		visit_children(clang_getTranslationUnitCursor(tu),|cur,_parent| {
			if cur.kind==CXCursor_MacroDefinition {
				...
				range.end_int_data-=if tokenize_range_adjust { 1 } else { 0 }; // clang bug for macros only
